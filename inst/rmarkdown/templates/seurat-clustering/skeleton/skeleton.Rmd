...
params:
  ...
  ## Use a filtered SingleCellExperiment object file as input.
  sce_file: "data/YYYY-MM-DD/sce_filtered.rds"
...
reduced_dim_groups <- unique(c(
    "ident", "sampleName", interestingGroups(sce), "Phase"
))
```
...
```{r coerce-sce-to-seurat, results="hide"}
...
cell_cycle_markers <- pointillism::cell_cycle_markers[[camel(organism)]]
...
```{r dims-use}
## Allow for user-defined PCs to use; otherwise, calculate using PC elbow plot.
...
```{r run-umap, include=has_umap, eval=has_umap}
seurat <- RunUMAP(seurat)
```

```{r plot-umap, include=has_umap, eval=has_umap, results="asis"}
invisible(lapply(
    X = reduced_dim_groups,
    FUN = function(group) {
        markdownHeader(group, level = 3, asis = TRUE, tabset = TRUE)
...
```{r plot-feature-umap, include=has_umap, eval=has_umap}
plotFeatureUMAP(
    object = seurat,
    features = features,
    dark = params$dark
...
```{r plot-feature-pcs}
## Limit to 9 PCs.
feature_pcs <- paste0("PC", head(dims_use, 9))
```

```{r plot-feature-pcs-tsne}
plotFeatureTSNE(
    object = seurat,
