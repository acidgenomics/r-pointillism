...
params:
  ...
  seurat_file: "data/YYYY-MM-DD/sce_filtered.rds"
  
  ## Name to use for Seurat object coerced from SingleCellExperiment.
  seurat_name: "seurat"
  ...
  ## Use dark mode theme (`acid_theme_dark()`) for plots.
  dark: FALSE
...
## The reticulate package can check to see if UMAP module is installed.
## This is required for `Seurat::RunUMAP()`.
stopifnot(reticulate::py_module_available(module = "umap"))
...
reduced_dim_groups <- unique(c(
    "ident", "sampleName", interestingGroups(object), "Phase"
))
```
...
data(cellCycleMarkersList, package = "pointillism")
cell_cycle_markers <- cellCycleMarkersList[[camelCase(organism)]]
stopifnot(is(cell_cycle_markers, "CellCycleMarkers"))
...
cell_cycle_genes <- sort(unique(c(s_genes, g2m_genes)))
```
    ...
    FUN = function(group) {
        ...
        lapply(
            ...
            FUN = function(res) {
                ...
                stopifnot(res_col %in% colnames(object[[]]))
                markdownHeader(res_col, level = 4, asis = TRUE, tabset = TRUE)
                Idents(object) <- res_col
                show(plotUMAP(
                    object = object,
...
if (!is.null(params$resolution_use)) {
    res_col <- paste0("RNA_snn_res.", params$resolution_use)
    print(res_col)
    Idents(object) <- res_col
}
```
...
```{r plot-feature-pcs}
## Limit to 6 PCs.
feature_pcs <- paste0("PC_", head(dims, n = 6L))
```
