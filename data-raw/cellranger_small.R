...
# FIXME Switch to splatter simulation data instead
...
sce <- splatter::splatSimulate()
hs <- makeGRangesFromEnsembl("Homo sapiens", release = 92)
rowRanges(sce) <- hs[seq_len(nrow(sce))]
metrics(counts(sce))
# FIXME Need to add force recalculate option, to use rowRanges
...
# known_markers_small ==========================================================
# FIXME The cellranger_small dataset needs some known markers
